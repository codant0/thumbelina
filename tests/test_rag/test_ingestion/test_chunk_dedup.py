...
class TestExactDedup:
    ...
    def test_exact_dedup_across_batches(self, engine: Engine):
        """与已有指纹匹配的 chunk 被过滤。"""
        dedup = ChunkDeduplicator(engine, enable_minhash=False)
...
class TestMinhashDedup:
    ...
    def test_minhash_near_duplicate_filtered(self, engine: Engine):
        """相似度 >= 阈值的 chunk 被过滤。"""
    ...
    def test_minhash_intra_batch_dedup(self, engine: Engine):
        """批次内两个近似 chunk 只保留第一个。"""
        dedup = ChunkDeduplicator(engine, jaccard_threshold=0.80)
...
class TestRegisterChunks:
    def test_register_then_dedup(self, engine: Engine):
        """注册 → 再次输入同批 chunk → 全被精确去重。"""
        dedup = ChunkDeduplicator(engine, enable_minhash=False)
...
class TestEdgeCases:
    ...
    def test_stats_accuracy(self, engine: Engine):
        ...
        new = [
            _make_chunk("已存在A", chunk_id="n1"),  # 跨批次重复
            _make_chunk("已存在B", chunk_id="n2"),  # 跨批次重复
        ...
        assert stats.exact_dup_count == 3  # n1, n2, n4
        assert stats.output_count == 2  # n3, n5
        assert stats.total_removed == 3
        assert len(result) == 2
...
class TestThreeTierIntegration:
    def test_exact_filters_before_minhash(self, engine: Engine):
        """SHA-256 精确去重在 MinHash 之前执行。"""
        dedup = ChunkDeduplicator(engine)
