...
- **Permission Modes** — Five-mode capability ladder enforced at the tool-execution gate (not a prompt convention): **read-only** (no local side effects) → **workspace write** (coder only, bounded to active workspace) → **global write** (any path; protected paths still flagged) → **full access** (default for chat; even protected paths allowed, only dangerous commands require approval) → **auto** (no confirmation, for unattended runs). Mode selector in the Web UI chat/coder toolbar (filtered by conversation type), persisted per conversation. A red approval card is mounted in the message stream whenever a tool call crosses the gate; user decision routes back to the agent via WebSocket. Unattended entry points (WeChat / QQ / Scheduler / HTTP / non-TTY CLI) **fail closed** — confirm-level calls are denied with no interrupt. Full replay of every tool call's verdict (allowed / confirmed / auto-allowed / denied) is shown on the Trajectory page. See `docs/specs/2026-09-06-permission-modes-design.md` for the rule matrix.
