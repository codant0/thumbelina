...
describe('llmConfig API', () => {
  ...
  it('fetchEndpoints returns parsed endpoints', async () => {
    vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify([{ id: '1', provider: 'openai', name: 'Default', base_url: 'https://api.openai.com/v1', models: ['gpt-4o'], api_key_set: true, is_default: true }]), { status: 200 }),
    )
    const endpoints = await fetchEndpoints()
    expect(endpoints).toHaveLength(1)
    expect(endpoints[0].name).toBe('Default')
    expect(endpoints[0].models).toEqual(['gpt-4o'])
  })

  it('createEndpoint sends api_key in body', async () => {
    const fetchSpy = vi.spyOn(globalThis, 'fetch').mockResolvedValue(
      new Response(JSON.stringify({ id: '1', provider: 'openai', name: 'Default', base_url: 'https://api.openai.com/v1', models: ['gpt-4o'], api_key_set: true, is_default: false }), { status: 201 }),
    )
