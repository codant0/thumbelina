...
describe('ToolCallsPanel', () => {
  it('按时间序列出本批全部调用与状态样式', () => {
    const { container } = render(<ToolCallsPanel toolCalls={batch} onClose={vi.fn()} />)
  ...
  it('close 按钮回调 onClose', () => {
    const onClose = vi.fn()
    const { container } = render(<ToolCallsPanel toolCalls={batch} onClose={onClose} />)
